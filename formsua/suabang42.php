...
<?php 
    ...
    $id = $_REQUEST['id'];
    $sql = "SELECT * FROM bang42 WHERE id = $id";
                        ...
                        <?php while($row = mysqli_fetch_assoc($result)){ ?>
                            <form action="" method="post" class="formNhap">
                                ...
                                <div class="container_box-content">
                                    ...
                                    <div class="container_box-input"><input class="content_input--item--input" min="0" type="number" name="tongnguonthu" placeholder="Tổng kinh phí" id="" value="<?= $row['tongnguonthu'] ?>" required></div>
                                </div>
                                <!-- tách -->
                            ...
                            <?php }
                                if(isset($_POST['sua'])){
                                    ...
                                    $sql = "UPDATE `bang42` SET `tongnguonthu` = '$tongnguonthu',`nam` = '$year' WHERE `bang42`.`id` = '$id'";
                                    $result = mysqli_query($conn, $sql);
                                    if($result){
                                        echo "<script>alert('Sửa thành công');</script>";
                                        echo '<script>window.location.href = "../formxem/bang42.php"</script>';
                                    }
                                    else{
