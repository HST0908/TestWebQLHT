...
$myexcel->getActiveSheet()->setTitle("bang21");
$myexcel->getActiveSheet()->setCellValue("A2", "BẢNG 21: TỔNG SỐ NGƯỜI HỌC ĐĂNG KÝ DỰ THI VÀO CSGD, TRÚNG TUYỂN VÀ NHẬP HỌC 
TRONG 5 NĂM GẦN ĐÂY HỆ CHÍNH QUY")->mergeCells('A2:H2');
...
$myexcel->getActiveSheet()->getStyle('A4:H4')->getFill()->setFillType(\PHPExcel_Style_Fill::FILL_SOLID);
$myexcel->getActiveSheet()->getStyle('A4:H4')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$myexcel->getActiveSheet()->getStyle('A2')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

$namhoc = $_POST['namin'];
$sql = "SELECT * FROM bang21 where namhoc = $namhoc";
...
$myexcel->getActiveSheet()->getStyle('A4:' . 'H'.($i))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$myexcel->getActiveSheet()->getStyle('A4:' . 'H' . ($i))->applyFromArray($styleArray);
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment;filename="bang21.xlsx"');
