...
$myexcel->getActiveSheet()->setTitle("bang48");
$myexcel->getActiveSheet()->setCellValue("A2", "BẢNG 48.  TỔNG CHI CHO HOẠT ĐỘNG KẾT NỐI DOANH NGHIỆP,TƯ VẤN VÀ HỖ TRỢ VIỆC LÀM")->mergeCells('A2:J2');
...
$sql = "SELECT * FROM bang48 where nam between $nambd and $namkt ORDER BY nam ASC";
...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $myexcel->getActiveSheet()->setCellValue("E" . $i, number_format($row['chiKNdoanhnghiep'], 0, '.','.'));
...
header('Content-Disposition: attachment;filename="bang48.xlsx"');
