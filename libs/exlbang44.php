...
$myexcel->getActiveSheet()->setTitle("bang44");
$myexcel->getActiveSheet()->setCellValue("A2", "BẢNG 44.  TỔNG CHI CHO HOẠT ĐỘNG NGHIÊN CỨU KHOA HỌC, CHUYỂN GIAO CÔNG NGHỆ VÀ PHỤC VỤ CỘNG ĐỒNG")->mergeCells('A2:J2');
$myexcel->getActiveSheet()->setCellValue("C4", "STT");
$myexcel->getActiveSheet()->setCellValue("D4", "Năm");
$myexcel->getActiveSheet()->setCellValue("E4", "Tổng chi");
...
$sql = "SELECT * FROM bang44 where nam between $nambd and $namkt ORDER BY nam ASC";
...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $myexcel->getActiveSheet()->setCellValue("E" . $i, number_format($row['chiNCKH'], 0, '.','.'));
...
header('Content-Disposition: attachment;filename="bang44.xlsx"');
