...
<body>
    ...
    $sql2 = "create table bangmoi(thanhtich char (100))";
    ...
    while ($row = mysqli_fetch_array($kq)) {
        ...
        for ($j = $nambd; $j <= $namkt; $j++) {
            $tam1 = $row[$i];
            $nam = "Nam" . $j;
            $sql2 = "update bangmoi set $nam=$tam1 where thanhtich = '$tam'";
            $kq2 = mysqli_query($con, $sql2);
            $i++;
        }
    }
	...
	if(mysqli_num_rows($rsout) > 0){
		$output.='
        <h5> NGHIÊN CỨU KHOA HỌC CỦA SINH VIÊN</h5>
        <table border="1">
		<tr>
		<td> Thành tích </td>';
    ...
    header('Content-Disposition:attachment;filename=bang38.xls');
    echo $output;
    ?>
</body>
