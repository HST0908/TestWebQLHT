...
$myexcel->getActiveSheet()->setTitle("bang46");
$myexcel->getActiveSheet()->setCellValue("A2", "BẢNG 46.  TỔNG CHI CHO HOẠT ĐỘNG ĐÀO TẠO")->mergeCells('A2:J2');
...
$sql = "SELECT * FROM bang46 where nam between $nambd and $namkt ORDER BY nam ASC";
...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $myexcel->getActiveSheet()->setCellValue("E" . $i, number_format($row['chiHDDT'], 0, '.','.'));
...
header('Content-Disposition: attachment;filename="bang46.xlsx"');
