...
$myexcel->getActiveSheet()->setCellValue("H4", "Năm học")->mergeCells("H4:H5");
...
$myexcel->getActiveSheet()->getStyle('B5:H5')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $myexcel->getActiveSheet()->setCellValue("H" . $i, $row['namhoc']);
    }
}
$styleArray = array(
    'borders' => array(
...
$myexcel->getActiveSheet()->getStyle('A6:' . 'H'.($i))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$myexcel->getActiveSheet()->getStyle('A4:' . 'H' . ($i))->applyFromArray($styleArray);
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment;filename="bang13b.xlsx"');
