...
$myexcel->getActiveSheet()->setTitle("bang42");
$myexcel->getActiveSheet()->setCellValue("A2", "BẢNG 42. TỔNG KINH PHÍ TỪ CÁC NGUỒN THU
CỦA TRƯỜNG ")->mergeCells('A2:J2');
$myexcel->getActiveSheet()->setCellValue("C4", "STT");
$myexcel->getActiveSheet()->setCellValue("D4", "Năm");
$myexcel->getActiveSheet()->setCellValue("E4", "Tổng kinh phí");
...
$sql = "SELECT * FROM bang42 where nam between $nambd and $namkt ORDER BY nam ASC";
...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $myexcel->getActiveSheet()->setCellValue("E" . $i, number_format($row['tongnguonthu'], 0, '.','.'));
...
header('Content-Disposition: attachment;filename="bang42.xlsx"');
