...
<body>
    ...
    $sql2 = "create table bangmoi(loaihoithao char (100))";
    ...
    $sql = $sql . " sum(if(nam=$i,soluong,0))as 'năm $i'";
    $sql = $sql . " FROM bang35 where nam between $nambd and $namkt GROUP BY loaihoithao DESC";
    $j = $nambd;
    ...
    while ($row = mysqli_fetch_array($kq)) {
        ...
        for ($j = $nambd; $j <= $namkt; $j++) {
            $tam1 = $row[$i];
            $nam = "Nam" . $j;
            $sql2 = "update bangmoi set $nam=$tam1 where loaihoithao = '$tam'";
            $kq2 = mysqli_query($con, $sql2);
            $i++;
        }
    }
    ...
    header('Content-Disposition:attachment;filename=bang35.xls');
    echo $output;
    ?>
</body>
